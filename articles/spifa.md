...
```
where $`\boldsymbol{B}`$ is the matrix of multivariate effects of the
predictors, $`\boldsymbol{T}`$ defines the relationship between the
spatial processes and the latent abilities, and $`\boldsymbol{v}_i`$ is
allowed to have a correlations structure $`\boldsymbol{R}`$.
