...
spmirt <- function (response, predictors = NULL, coordinates = NULL,
  ...
  c_initial <-
    check_param_vec(c_opt, "initial", nitems, rnorm(nitems,c_prior_mean,c_prior_sd))
  ...
  A_prior_sd <-
    check_param_mat2(A_opt, "prior_sd", c(nitems, nfactors),
                     1 - diag(0.55, nitems, nfactors))
  A_initial <-
  ...
  if (is.null(coordinates)) {
    ...
    sigmas_gp_initial <-
      check_param_vec(sigmas_gp_opt, "initial", nfactors, sigmas_gp_mean)
  ...
  #   constrain_L = constrain_L, constrain_T = constrain_T,
  ...
  #   phi_gp_initial = phi_gp_initial, phi_gp_mean = phi_gp_mean,
  #   phi_gp_sd = phi_gp_sd,
  ...
  #             adap_Sigma_gp_phi = adap_Sigma_gp_phi, model_type,
  #             adap_Sigma = adap_Sigma, adap_scale, adap_C, adap_alpha, adap_accep_prob))
  # return(list(R_initial, R_prior_eta))
  return(auxout)
}
...
check_param_matdiag <- function (param_list, element, dimension, default) {
  ...
  if (is.null(param_list[[element]])) {
    ...
    output <- diag(param_list[[element]], dimension, dimension)
  } else if (length(param_list[[element]]) == dimension) {
