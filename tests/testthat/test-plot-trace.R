...
test_that("plot_trace(): selects by group name or explicit indexed names", {
  data(ipixuna, package = "spifa")
  samples <- spifa(items ~ 1, data = ipixuna, nfactors = 3, ngp = 0, niter = 20)

  gg_group <- plot_trace(samples, select = "c", facet = TRUE)
  ...
  expect_equal(
    sort(as.character(unique(gg_explicit$data$parameter))),
    paste0("c[", 1:3, "]")
  )
})

test_that("plot_trace(): orders parameters by index, not alphabetically", {
  data(ipixuna, package = "spifa")
  samples <- spifa(items ~ 1, data = ipixuna, nfactors = 3, ngp = 0, niter = 20)
...
test_that("plot_trace(): drops restricted A parameters", {
  data(ipixuna, package = "spifa")
  ...
  samples <- spifa(
    items ~ 1, data = ipixuna, nfactors = nfactors, ngp = 0, niter = 20,
...
test_that("plot_trace(): nshow randomly subsamples parameters", {
  data(ipixuna, package = "spifa")
  samples <- spifa(items ~ 1, data = ipixuna, nfactors = 3, ngp = 0, niter = 20)
...
test_that("plot_trace(): overlay legend", {
  data(ipixuna, package = "spifa")
  nitems <- ncol(ipixuna$items)
  nfactors <- 3
  A <- matrix(1, nitems, nfactors)
...
test_that("plot_trace(): plotmath-parses single and two-index parameter names", {
  data(ipixuna, package = "spifa")
