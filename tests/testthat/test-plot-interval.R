...
test_that("plot_interval(): selects by group name or explicit indexed names", {
  data(ipixuna, package = "spifa")
  samples <- spifa(items ~ 1, data = ipixuna, nfactors = 3, ngp = 0, niter = 20)

  gg_group <- plot_interval(samples, select = "c")
...
test_that("plot_interval(): orders parameters by index, not alphabetically", {
  data(ipixuna, package = "spifa")
  samples <- spifa(items ~ 1, data = ipixuna, nfactors = 3, ngp = 0, niter = 20)
...
test_that("plot_interval(): drops restricted A parameters", {
  data(ipixuna, package = "spifa")
...
test_that("plot_interval(): nshow randomly subsamples parameters", {
  data(ipixuna, package = "spifa")
  samples <- spifa(items ~ 1, data = ipixuna, nfactors = 3, ngp = 0, niter = 20)
  ...
  # NULL (the default) shows every matched parameter
...
test_that("plot_interval(): plotmath-parses single and two-index parameter names", {
  data(ipixuna, package = "spifa")

  samples <- spifa(items ~ 1, data = ipixuna, nfactors = 3, ngp = 1, niter = 20)

  # two-index "A[i,j]" becomes "A[list(i,j)]"
  gg_a <- plot_interval(samples, select = "A[1,1]")
  expect_equal(as.character(unique(gg_a$data$parameter)), "A[list(1,1)]")

  # single-index "phi[1]" is left as-is
  gg_phi <- plot_interval(samples, select = "phi")
  expect_equal(as.character(unique(gg_phi$data$parameter)), "phi[1]")
})

test_that("plot_interval(): reference accepts a matrix matching a group's own shape", {
  data(ipixuna, package = "spifa")
  nitems <- ncol(ipixuna$items)
  nfactors <- 3
  samples <- spifa(items ~ 1, data = ipixuna, nfactors = nfactors, ngp = 0, niter = 20)
...
test_that("plot_interval(): reference accepts a plain vector matching a group's shape", {
  data(ipixuna, package = "spifa")
  nitems <- ncol(ipixuna$items)
  samples <- spifa(items ~ 1, data = ipixuna, nfactors = 3, ngp = 0, niter = 20)
...
test_that("plot_interval(): reference requires select to be a single group name", {
  data(ipixuna, package = "spifa")
  samples <- spifa(items ~ 1, data = ipixuna, nfactors = 3, ngp = 0, niter = 20)
