# Package index

## All functions

- [`dic()`](https://ErickChacon.github.io/spifa/reference/dic.md) :
  Deviance Information Criterion
- [`dic(`*`<spifa>`*`)`](https://ErickChacon.github.io/spifa/reference/dic.spifa.md)
  : Deviance Information Criterion for a spifa Model
- [`ipixuna`](https://ErickChacon.github.io/spifa/reference/ipixuna.md)
  : Simulated Food Insecurity Survey Data for Ipixuna
- [`plot(`*`<spifa>`*`)`](https://ErickChacon.github.io/spifa/reference/plot.spifa.md)
  : Default Plot of spifa Posterior Samples
- [`plot_density()`](https://ErickChacon.github.io/spifa/reference/plot_density.md)
  : Density Plot of spifa Posterior Samples
- [`plot_interval()`](https://ErickChacon.github.io/spifa/reference/plot_interval.md)
  : Interval Plot of spifa Posterior Samples
- [`plot_predict()`](https://ErickChacon.github.io/spifa/reference/plot_predict.md)
  : Map Posterior Predictions from a Fitted spifa Model
- [`plot_trace()`](https://ErickChacon.github.io/spifa/reference/plot_trace.md)
  : Trace Plot of spifa Posterior Samples
- [`predict(`*`<spifa>`*`)`](https://ErickChacon.github.io/spifa/reference/predict.spifa.md)
  : Predict the Latent Factors of a spifa Model
- [`print(`*`<spifa>`*`)`](https://ErickChacon.github.io/spifa/reference/print.spifa.md)
  : Print spifa Posterior Samples
- [`spifa-package`](https://ErickChacon.github.io/spifa/reference/spifa-package.md)
  : spifa: Bayesian Spatial Item Factor Analysis
- [`spifa()`](https://ErickChacon.github.io/spifa/reference/spifa.md) :
  Bayesian Spatial Item Factor Analysis
- [`summary(`*`<spifa>`*`)`](https://ErickChacon.github.io/spifa/reference/summary.spifa.md)
  : Summarise spifa Posterior Samples
- [`update(`*`<spifa>`*`)`](https://ErickChacon.github.io/spifa/reference/update.spifa.md)
  : Continue Sampling from the Posterior of a spifa Model
